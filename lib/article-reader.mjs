  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    ...
    try {
      response = await fetch(currentUrl, {
        ...
        headers: {
          'Accept': acceptPdf
            ? 'text/html,application/xhtml+xml,application/pdf;q=0.95,*/*;q=0.1'
            : 'text/html,application/xhtml+xml;q=0.9,*/*;q=0.1',
          'Accept-Language': 'ja,en-US;q=0.8,en;q=0.6',
          'User-Agent': 'PersonalNewsSummary/2.0 (server-side article/PDF reader)'
        }
      });
    } catch (err) {
    ...
    const looksPdf = contentType.includes('application/pdf') || currentUrl.pathname.toLowerCase().endsWith('.pdf');

    if (looksPdf) {
      if (!acceptPdf) throw new Error('PDFはこの取得処理では許可されていません');
      const buffer = await readResponseBufferLimited(response, MAX_PDF_BYTES, 'PDF');
      return { kind: 'pdf', buffer, finalUrl: currentUrl.href, contentType };
    }

    if (contentType && !contentType.includes('text/html') && !contentType.includes('application/xhtml+xml')) {
      throw new Error('記事リンクがHTML/PDFではありません');
    }

    const buffer = await readResponseBufferLimited(response, MAX_HTML_BYTES, '記事HTML');
    return { kind: 'html', buffer, finalUrl: currentUrl.href, contentType };
  }
...
function findJStagePdfUrl(htmlBuffer, pageUrl) {
  let page;
  try { page = new URL(pageUrl); } catch { return ''; }
  if (!/(^|\.)jstage\.jst\.go\.jp$/i.test(page.hostname)) return '';
  ...
  try {
    ...
    const meta = document.querySelector('meta[name="citation_pdf_url"], meta[property="citation_pdf_url"]');
    if (meta?.content) candidates.push(meta.content);

    document.querySelectorAll('link[type="application/pdf"][href], a[href]').forEach(node => {
      const href = node.getAttribute('href') || '';
      const label = normalizeText(node.textContent || '').toLowerCase();
      if (
        /\/_pdf\//i.test(href) ||
        /\.pdf(?:$|[?#])/i.test(href) ||
        /pdfをダウンロード|pdf download|download pdf/i.test(label)
      ) {
        candidates.push(href);
      }
    });

    if (page.pathname.includes('/_article/')) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (/(^|\.)jstage\.jst\.go\.jp$/i.test(resolved.hostname)) return resolved.href;
      } catch {}
    }
    return '';
  } finally {
...
export async function extractArticleFromUrl(rawUrl, options = {}) {
  ...
  const resource = await fetchResourceSafely(url, { acceptPdf: true });
  ...
  let pdfError = '';
  if (preferPdf) {
    const pdfUrl = findJStagePdfUrl(resource.buffer, resource.finalUrl);
    if (pdfUrl) {
      try {

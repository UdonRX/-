...
function describeGeminiApiError(status, data) {
  ...
  if (status === 404) {
    return {
      ...
      detail: apiMessage || 'このAPIキーのプロジェクトで利用可能なモデルを確認してください。推奨: gemini-3.1-flash-lite'
    };
  }
...
export async function generateGemini({
  ...
  if (responseSchema) {
    // Gemini generateContent の新しい responseFormat では
    // mimeType は MIME文字列ではなく enum 値を指定する。
